...
namespace Proyecto2025.Server.Controllers
{
    ...
    public class TripulanteController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<TripulanteListadoDTO>>> GetListaTripulante()
        {
        ...
        [HttpPost]
        public async Task<IActionResult> CrearTripulanteConPersona([FromBody] TripulanteConPersonaDTO dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            try
            {
                var tripulanteId = await repositorio.InsertarTripulanteConPersona(dto);
                return Ok(new { TripulanteId = tripulanteId });
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError,
                    new { message = "Error al crear el tripulante", detail = ex.Message });
            }
        }

        [HttpPut("actualizarEstadoMovil/{tripulanteId}")]
        public async Task<IActionResult> ActualizarEstadoEnMovil(int tripulanteId, [FromQuery] bool enMovil)
        {
            try
            {
                await repositorio.ActualizarEstadoEnMovil(tripulanteId, enMovil);
                return Ok(new { message = $"Estado 'EnMovil' actualizado a {enMovil} para el tripulante {tripulanteId}." });
            }
            catch (Exception ex)
            {
