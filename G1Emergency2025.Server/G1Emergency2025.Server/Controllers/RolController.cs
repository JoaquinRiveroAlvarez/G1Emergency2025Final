...
namespace Proyecto2025.Server.Controllers
{
    ...
    public class RolController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<List<RolListadoDTO>>> GetList()
        {
        ...
        [HttpGet("Codigo/{cod}")]
        public async Task<ActionResult<RolDTO>> GetByCod(string cod)
        {
        ...
        [HttpGet("ListaRol")]
        public async Task<ActionResult<List<RolListadoDTO>>> GetListaRol()
        {
            var lista = await repositorio.SelectListaRol();
            if (lista == null)
            {
                return NotFound("No se encontro la lista, VERIFICAR.");
            }
        ...
        [HttpPost]
        public async Task<ActionResult<int>> Post(RolDTO DTO)
        {
            try
            {
                Rol entidad = new Rol
        ...
        [HttpPut("{id:int}")]
        public async Task<ActionResult> Put(int id, RolDTO DTO)
        {
            var entidad = new Rol
            {
                Id = id,
